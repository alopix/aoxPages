...
class OutlineTpl implements IOutlineEngine {
  ...
  protected function initEngine() {
    ...
    */
	...
	/**
	 * three new functions to deal with global template variables
	 * specialty: array with values can be sumbmitted
	 */
	public static function globalAssign() {
		$argc = func_num_args();
		$var = func_get_arg(0);
		if ($argc == 2) {
			self::$globalVars[$var] = func_get_arg(1);
		} else if ($argc == 1 && is_array($var)) {
	...
	public static function globalAssignByRef($var, &$value) {
		$argc = func_num_args();
		$var = func_get_arg(0);
		if ($argc == 2) {
			self::$globalVars[$var] = &func_get_arg(1);
	...
	protected function retrieveGlobalVars() {
		foreach (self::$globalVars AS $var => $value) {
			$this->vars[$var] = $value;
		}
	}
	/**
  ...
  public function clear_cache() {
    ...
    */
    ...
    /**
     * change the static variables to normal ones
     */
    $this->retrieveGlobalVars();
